...
function relabel_bands(label, indices, coeffs; full_single_particle::Bool=false)
    ...
    if length(band_pos) == 2 ## This is a one-body term 
...
function matrix_elements_to_diagham(label, indices, coeffs, file_name::String; dropband::Bool=false, full_single_particle::Bool=false)
